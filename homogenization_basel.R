...
inpath <- "../Basel/SEF/5_monthly"
dailypath <- "../Basel/SEF/4_daily"
refpath <- "../Basel/SEF/refs"
outpath <- "../Basel/SEF/6_hom"

target_series <- c("Merian_1837", "Kaufmann", "Schneider", "Merian_1833", "Merian_1828",
                   "Delemont", "Fuerstenberger_182810", "Huber_1817", "Fuerstenberger_182801", 
                   "Fuerstenberger_1826", "Burckhardt", "Mulhouse", "Annone", "Huber_1789", 
                   "Socin", "Wolleb", "Ryhiner", "Zwinger")

ref_series <- c("Solothurn", "Weissenstein", "Vevey", "StGallen",
                "Gottstatt", "Fribourg", "Simler",
                "Neuchatel", "Schaffhausen",
                "Herisau_Nef", "Fontaines", "Morges", "Herisau_Merz", "Luzern_Ineichen",
                "Aarau_Bronner", "Zug", "Uetliberg", "Bern_Observatory", "Reinhard", 
                "Burgdorf_1851", "Koch", "Wolf", "Benoit", "Trechsel_1848", "Trechsel_1826", 
                "Studer_1803", "Studer_1801", "Sutz",
                "Studer_1797", "Bueren", "Lombach_1785", "Lombach_1777", "Studer_1779", "Tavel",
                "Zurich_Observatory", "GCOS_Winterthur", "Kuesnacht", "Waldenburg",
                "NGZ_1842", "NGZ_1836", "Hofmeister", "Horner_1823", "SNG_1831", "Feer", "Escher_181601", 
                "Horner_1812", "Hirzel_1795", "Muralt_1760", "Hirzel_1761", "Meyer_1759", "Ott",
                target_series)

nmin <- 60 # minimum overlap in months above which reference series are not used

breakpoints <- read.csv("../Metadata/Basel_breakpoints_ta.csv", sep=";", stringsAsFactors=FALSE)
breakpoints_ref <- read.csv("../Metadata/Refs_breakpoints_ta.csv", sep=";", stringsAsFactors=FALSE)

## Remove unused breakpoints in Huber
ko <- which(breakpoints$Station=="Huber" & 
              year(as.Date(breakpoints$Date,format="%d.%m.%Y"))%in%1798:1817)
if (length(ko) > 0) breakpoints <- breakpoints[-ko, ]
...
## Initialize list of homogeneous segments
x <- list()
...
for (i in 2:length(x)) {
  ...
  if (x[[i]]$station[1] != last_station) { # write homogenized daily file and read in the next to homogenize
    ...
    } else if (x[[i]]$station[1] != x[[1]]$station[1]) {
