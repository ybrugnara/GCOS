...
inpath <- "../Bern/SEF/5_monthly"
dailypath <- "../Bern/SEF/4_daily"
refpath <- "../Bern/SEF/refs"
outpath <- "../Bern/SEF/6_hom"

target_series <- c("Bern_Observatory", "Reinhard", "Burgdorf_1857", "Koch", "Wolf",
                   "Burgdorf_1851", "Benoit", "Trechsel_1848", "Trechsel_1826", "Studer_1803", 
                   "Fueter_1819", "Fueter_1803", "Studer_1801", "Sutz", "Studer_1797", 
                   "Bueren", "Lombach_1785", "Lombach_1777", "Studer_1779", "Tavel") # NB: the first series will not be homogenized

ref_series <- c("Delemont", "Solothurn", "Weissenstein", "Vevey", "StGallen", "Avully",
                "Beatenberg", "Saanen", "Gottstatt", "Fribourg", "Aarau_Zschogge", "Simler",
                "Annone", "Neuchatel", "Huber_1789", "Mulhouse", "Schaffhausen", "Geneve",
                "DigiHom_Geneva_1798", "DigiHom_Geneva_1822", "DigiHom_Geneva_1826", "DigiHom_Geneva_1836",
                "Socin", "Wolleb", "Ryhiner", "JdG", "Genthod_1789", "Genthod_1796",
                "Herisau_Nef", "Fontaines", "Morges", "Herisau_Merz", "Luzern_Ineichen",
                "Aarau_Bronner", "Zug", "Gotthard", "Uetliberg", "Merian_1828", "Merian_1833", "Merian_1837",
                "Zurich_Observatory", "GCOS_Winterthur", "Bruegger", "Kuesnacht", "Waldenburg",
                "NGZ_1842", "NGZ_1836", "Hofmeister", "Horner_1823", "SNG_1831", "Feer", "Escher_181601", 
                "Horner_1812", "Hirzel_1795", "Muralt_1760", "Hirzel_1761", "Meyer_1759", "Ott", target_series)

nmin <- 60 # minimum overlap in months above which reference series are not used

breakpoints <- read.csv("../Metadata/Bern_breakpoints_ta.csv", sep=";", stringsAsFactors=FALSE)
...
for (i in 2:length(x)) {
  ...
  if (x[[i]]$station[1]!=last_station) { # write homogenized daily file and read in the next to homogenize
    ...
    x_daily$dates <- as.Date(paste(x_daily$Year,x_daily$Month,x_daily$Day,sep="-"), format="%Y-%m-%d")
  ...
  if (x[[i]]$station[1] == "Reinhard") { # use parallel observations for Reinhard
    message("Using parallel observations by Simler")
    j <- 0
    found <- FALSE
    while (j < length(y) & !found) {
      j <- j + 1
      if (y[[j]]$station[1] == "Simler") found <- TRUE
    }
    xpar <- x[[i]][x[[i]]$dates%in%y[[j]]$dates, ]
    par_adjustments <- c(rep(NA,5), y[[j]]$Value-xpar$Value, NA)
    adjustments <- rbind(adjustments, par_adjustments)
  }
...
## Write Tavel monthly file
out <- x[[i-2]]
out$Hour <- out$Minute <- NA
out <- out[,c("Year","Month","Day","Hour","Minute","Value_hom")]
meta <- read_meta(target_files[grep(last_station,target_files)])
write_sef(out, outpath, meta["var"], meta["id"], meta["name"], meta["lat"], 
          meta["lon"], meta["alt"], meta["source"], meta["link"], meta["units"],
